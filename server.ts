  ...
  app.get('/:username/:subName', async (req, res, next) => {
    ...
    try {
      ...
      if (isBrowser) {
        ...
        let qrCodeImageUrl = '';
        try {
          qrCodeImageUrl = await QRCode.toDataURL(subUrl);
        } catch (qrErr) {
          console.error("Failed to generate custom QR code for user sub", qrErr);
          qrCodeImageUrl = 'https://i.ibb.co/7J85dCT8/Untitled.png';
        }
        ...
        return res.send(`
          ...
          <html>
          <head>
            <title>${foundSub.name || 'Подписка'} | vlessfree Sub</title>
          ...
          <body>
            <div class="card">
              ...
              <div style="margin-bottom: 24px; text-align: center;">
                <h2 style="font-size: 20px; font-weight: bold; margin: 0 0 8px 0; color: #4ade80;">📁 Папка-подписка пользователей: ${foundSub.name}</h2>
                <div style="font-size: 11px; opacity: 0.5; font-family: monospace;">Автор: @${foundSub.username}</div>
              </div>

              <div class="container">
                <div class="left-side">
                  <p>Скопируйте ссылку подписки и вставьте её в клиент или выберите приложение ниже для быстрого импорта:</p>
                ...
                <div class="right-side">
                  <div class="qr-image">
                    <img src="${qrCodeImageUrl}" alt="QR Code" style="width: 180px; height: 180px; display: block; object-fit: contain; background: #fff; padding: 4px; border-radius: 12px;">
                  </div>
